(function (window, document, undefined) {
    ...
    L.DomUtil.setTransform = function (el, point, angle, disable3D) {
      ...
      if (!disable3D && L.Browser.any3d) {
        ...
        el.style[L.DomUtil.TRANSFORM] =
          el.style[L.DomUtil.TRANSFORM] +
          " " +
          L.DomUtil.getRotateString(angle, "rad");
      } else {
  ...
  L.Map.include({
    _newLayerPointToLatLng: function (point, newZoom, newCenter) {
      var topLeft = L.Map.prototype._getNewTopLeftPoint
  ...
  L.Illustrate.Pointer = L.Path.extend({
    ...
    _getLatLngs: function () {
      ...
      for (var i = 0, l = this._coordinates.length; i < l; i++) {
        latlngs[i] = this._map.layerPointToLatLng(
          this._coordinates[i].add(origin)
        );
      }
  ...
  L.Illustrate.Pointer = L.Illustrate.Pointer.extend({
    ...
    _animateZoom: function (opt) {
      var anchor = this._map.latLngToLayerPoint(this._latlng),
        newAnchor = this._map._latLngToNewLayerPoint(
          this._latlng,
          opt.zoom,
          opt.center
        ),
      ...
      this._pathRoot.style[L.DomUtil.TRANSFORM] = L.DomUtil.getTranslateString(
        this._pathViewport.min.add(offset)
      );
  ...
  L.Illustrate.Create.Pointer = L.Draw.Polyline.extend({
    ...
    _fireCreatedEvent: function () {
      ...
      for (var i = 0, length = latlngs.length; i < length; i++) {
        coordinates[i] = this._map
          .latLngToLayerPoint(latlngs[i])
          ._subtract(origin);
      }

      pointer = new L.Illustrate.Pointer(
        latlngs[0],
        coordinates,
  ...
  L.Illustrate.Create.Textbox = L.Draw.Rectangle.extend({
    ...
    initialize: function (map, options) {
      this.options.textOptions = L.extend(
        {},
    ...
    _fireCreatedEvent: function () {
      var latlngs = this._shape.getLatLngs(),
        ...
        oppositeCornerPixelCoordinates = this._map
          .latLngToLayerPoint(oppositeCorner)
          .round(),
        width = oppositeCornerPixelCoordinates.x - cornerPixelCoordinates.x + 2,
        height =
          oppositeCornerPixelCoordinates.y - cornerPixelCoordinates.y + 2;

      var textbox = new L.Illustrate.Textbox(
        center,
        this.options.textOptions
  ...
  L.Illustrate.EditHandle = L.RotatableMarker.extend({
    ...
    initialize: function (shape, options) {
      ...
      this._handleOffset = new L.Point(
        options.offset.x || 0,
        options.offset.y || 0
      );
      this._handled = shape;

      var latlng = this._handled._map.layerPointToLatLng(
    ...
    _animateZoom: function (opt) {
      var map = this._handled._map,
        handleLatLng = map._newLayerPointToLatLng(
          this._textboxCoordsToLayerPoint(this._handleOffset, opt),
          opt.zoom,
          opt.center
        ),
        pos = map
          ._latLngToNewLayerPoint(handleLatLng, opt.zoom, opt.center)
          .round();
    ...
    _latLngToTextboxCoords: function (latlng, opt) {
      ...
      return this._layerPointToTextboxCoords(
    ...
    _textboxCoordsToLatLng: function (coord, opt) {
      ...
      return map.layerPointToLatLng(
        this._textboxCoordsToLayerPoint(coord, opt)
      );
    },
  });
  ...
  L.Illustrate.RotateHandle = L.Illustrate.EditHandle.extend({
    ...
    _onHandleDrag: function (event) {
      var handle = event.target,
        ...
        point = this._map
          ...
          .subtract(this._map.latLngToLayerPoint(center)),
        theta;
    ...
    _createPointer: function () {
      ...
      this._pointer = new L.Illustrate.Pointer(
        this._handled.getLatLng(),
        [],
        options
      );
      this._updatePointer();
  ...
  L.Illustrate.Edit.Pointer = L.Edit.Poly.extend({
    ...
    _initHandles: function () {
      if (!this._handleGroup) {
        ...
        this._shape.getRotation = function () {
          return 0;
        };
    ...
    _removeVertex: function (handle) {
      ...
      pointer.fire("edit:remove-vertex", {
        handle: handle,
        removedId: removedId,
      });
